  ...
  public static void main(String[] args) {
    ...
    Thread t3 = new Thread(() -> {
      try {
        SEMAPHORE_1.acquire();
        SEMAPHORE_2.acquire();
        System.out.println(YELLOW + "Активность 3 запускается после 1 и 2" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(YELLOW + "Активность 3 завершена после 1 и 2" + RESET);
      SEMAPHORE_3.release();
    });
    ...
    Thread t4 = new Thread(() -> {
      try {
        SEMAPHORE_2.acquire();
        SEMAPHORE_3.acquire();
        System.out.println(BLUE + "Активность 4 запускается после 2 и 3" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(BLUE + "Активность 4 завершена после 2 и 3" + RESET);
    ...
    Thread t5 = new Thread(() -> {
      try {
        SEMAPHORE_4.acquire();
        System.out.println(PURPLE + "Активность 5 запускается после 4" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(PURPLE + "Активность 5 завершена после 4" + RESET);
      SEMAPHORE_5.release();
    });
    ...
    Thread t6 = new Thread(() -> {
      try {
        SEMAPHORE_4.acquire();
        SEMAPHORE_5.acquire();
        System.out.println(WHITE + "Активность 6 запускается после 4 и 5" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(WHITE + "Активность 6 завершена после 4 и 5" + RESET);
    });
