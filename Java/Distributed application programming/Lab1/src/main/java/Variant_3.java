...
public class Variant_3 {
  ...
  public static void main(String[] args) {
    ...
    Thread t3 = new Thread(() -> {
      try {
        SEMAPHORE_2.acquire();
        System.out.println(YELLOW + "Активность 3 запускается после 2" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(YELLOW + "Активность 3 завершена после 2" + RESET);
      SEMAPHORE_3.release();
    });
    ...
    Thread t4 = new Thread(() -> {
      try {
        SEMAPHORE_3.acquire();
        System.out.println(BLUE + "Активность 4 запускается после 3" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(BLUE + "Активность 4 завершена после 3" + RESET);
      SEMAPHORE_4.release();
    });
    ...
    Thread t6 = new Thread(() -> {
      try {
        SEMAPHORE_5.acquire();
        System.out.println(CYAN + "Активность 6 запускается после 5" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(CYAN + "Активность 6 завершена после 5" + RESET);
    });
