...
public class Variant_1 {
  ...
  public static void main(String[] args) {
    ...
    Thread t3 = new Thread(() -> {
      System.out.println(YELLOW + "Активность 3 начинается" + RESET);
      try {
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
    ...
    Thread t4 = new Thread(() -> {
      try {
        ...
        SEMAPHORE_2.acquire();
        SEMAPHORE_3.acquire();
        System.out.println(BLUE + "Активность 4 запускается после 1, 2, 3" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(BLUE + "Активность 4 завершена после 1, 2, 3" + RESET);
      SEMAPHORE_4.release(3);
    });
    ...
    Thread t6 = new Thread(() -> {
      try {
        SEMAPHORE_4.acquire();
        System.out.println(CYAN + "Активность 6 запускается после 4" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(CYAN + "Активность 6 завершена после 4" + RESET);
    });
