...
public class Variant_7 {
  ...
  public static void main(String[] args) {
    ...
    Thread t6 = new Thread(() -> {
      try {
        SEMAPHORE_2.acquire();
        System.out.println(WHITE + "Активность 6 запускается после 2" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(WHITE + "Активность 6 завершена после 2" + RESET);
    });

    Thread t7 = new Thread(() -> {
      try {
        SEMAPHORE_3.acquire();
        System.out.println(CYAN + "Активность 7 запускается после 3" + RESET);
        Thread.sleep(1000);
      } catch (InterruptedException ignored) {
      }
      System.out.println(CYAN + "Активность 7 завершена после 3" + RESET);
    });
